...
class SentiAttn(nn.Module):
    ...
    def __init__(self, input_size, embed_size=768, channels=100, fc_input_size=200, hidden_size = 500, output_size = 50, latent_v_len = 600, fm_k = 8):
    ...
    def forward(self, user_id, item_id, x_user_pos, x_user_neg, x_item_pos, x_item_neg, user_pos_senti, user_neg_senti, item_pos_senti, item_neg_senti):
        ...
        z = torch.cat([out_user, user_emb, out_item, item_emb], 1)
        
        #print(z.size())
        out = self.fmlayer(z) + self.user_bias(user_id) + self.item_bias(item_id)
        # user = torch.cat([out_user, user_emb], 1)
        # item = torch.cat([out_item, item_emb], 1)
