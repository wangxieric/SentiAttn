...
train_loader = data_loader.get_loader('office_products_train.p', batch_size)
val_loader = data_loader.get_loader('office_products_valid.p', batch_size, shuffle=False)
test_loader = data_loader.get_loader('office_products_test.p', batch_size, shuffle=False)
...
SentiAttn = model_1D.SentiAttn(input_size)
# print(SentiAttn)
...
for epoch in range(num_epochs):
    for i, (user_id, item_id, user_pos, user_neg, item_pos, item_neg, user_pos_senti, user_neg_senti, item_pos_senti, item_neg_senti, labels) in enumerate(train_loader):
        ...
        if i%100 ==0:
            ...
            batch_loss = 0
    # Save the Model
    torch.save(SentiAttn.state_dict(), 'result/office_products/1D-2/model_'+str(epoch)+'.pkl')
    print("==================================================================================")
    print("Testing Start..")
    ...
    pickle.dump((target, result),open('result/office_products/1D-2/result_test'+ str(epoch) + 'office_product.pickle','wb'))
